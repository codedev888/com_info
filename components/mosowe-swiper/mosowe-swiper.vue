	...
	<view class="mosowe-swiper" :style="'height:'+height+'rpx'">
		...
		<!-- #ifndef MP-WEIXIN -->
				<swiper-item
					...
					<!-- 仅文字展示 -->
					<text  class="text flex-row text-box" v-if="swiperType === 'text'">
						<text class="title">{{textKey ? item[textKey] : item.title}}</text>
						<text class="ld">{{item.s}}</text>
						<text class="hot">{{item.s}}</text>
					</text>
					<!-- 图文左右轮播 -->
...
<style lang="scss">
/* .text-box text{display: flex;justify-content: space-between; width: 100%; }
.text-box span{display: flex;justify-content: space-between; width: 100%;} */
.mosowe-swiper{
	width: 100%;
	position: relative;
	.uni-swiper {
	...
	.text-box{justify-content: space-between; width: 100%; display:flex}
	.title{width: 60%;	
		display: block;
	overflow: hidden;
		text-overflow: ellipsis;
